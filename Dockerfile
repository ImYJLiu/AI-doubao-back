...
# --- 2. 从华为云镜像下载 Python 3.12 源码并编译（不加 --enable-optimizations 避免链接问题） ---
RUN cd /tmp && \
...
# 注意：部分依赖（如 controlnet-aux）会引入 opencv-python 冲突，后续步骤会清理
...
# --- 6.1 清理 OpenCV 冲突（部分依赖会引入 opencv-python GUI 版，与 headless 冲突） ---
